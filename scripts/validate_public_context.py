...
if not isinstance(x.get('works_history'),list): errors.append('works_history missing')
...
print(f"OK schema=2 status={x.get('status')} works={len(x.get('works',[]))} works_history={len(x.get('works_history',[]))} parking={len(x.get('parking',[]))} sources={len(x.get('source_health',[]))}")
