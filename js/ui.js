...
AU.ui = (() => {
  ...
  function causalBadge(c){
    ...
    const cls={strong:'critical',moderate:'warning',weak:'info',none:'muted',insufficient:'muted'}[c.status]||'muted';
    return `<span class="causal-badge ${cls}">Contexte Clermont · ${U().escapeHtml(c.label||'testé')} ${c.score?`· ${U().integer(c.score)}/100`:''}</span>`;
  }

  function causalEvidence(c){
    if(!c||!c.tested) return '';
    const chain=(c.chain||[]).map(x=>`<li>${U().escapeHtml(x)}</li>`).join('');
    const works=(c.works||[]).slice(0,4).map(w=>`<li><strong>${U().escapeHtml(w.place||w.sector||'Travaux')}</strong> · compatibilité événement ${U().integer(w.compatibility||0)}/100 · ${U().escapeHtml(w.severityReason||'contexte public')}<br><span class="muted">${U().escapeHtml(w.text||'')}</span></li>`).join('');
    const alternatives=(c.alternatives||[]).slice(0,4).map(a=>`<li>${U().escapeHtml(a.label)} · signal ${U().integer(a.strength||0)}/100 — ${U().escapeHtml(a.evidence||'')}</li>`).join('');
    return `<div class="causal-box ${U().escapeHtml(c.status||'none')}"><div class="causal-head">${causalBadge(c)}</div><p>${U().escapeHtml(c.summary||'')}</p>${chain?`<h4>Chaîne explicative automatique</h4><ol>${chain}</ol>`:''}${works?`<h4>Événements publics testés</h4><ul>${works}</ul>`:''}${alternatives?`<h4>Causes concurrentes testées</h4><ul>${alternatives}</ul>`:''}</div>`;
  }
  ...
  function contextView(model,root){
    ...
    if(!c){root.innerHTML=viewHeader('Contexte causal','Couplage automatique diagnostics ↔ travaux ↔ mobilité')+'<section class="panel"><div class="empty-mini">Le moteur contextuel n’a pas encore été exécuté.</div></section>';return;}
    const top=c.top||[]; const events=c.events||[];
    root.innerHTML=viewHeader('Contexte causal Clermont','Chaque diagnostic métier est automatiquement testé contre les secteurs, la chronologie des travaux, les zones témoins et les causes concurrentes',qualityPill(c.strong?'estimate':'calculated',`${c.tested} tests exécutés`))+
      `<div class="quality-grid"><div class="quality-card"><span>Diagnostics testés</span><strong>${U().integer(c.tested)}</strong><span>hors qualité des données</span></div><div class="quality-card"><span>Compatibilités fortes</span><strong>${U().integer(c.strong)}</strong><span>score ≥ 78/100</span></div><div class="quality-card"><span>Compatibilités moyennes</span><strong>${U().integer(c.moderate)}</strong><span>score 58–77/100</span></div><div class="quality-card"><span>Couverture contextuelle</span><strong>${U().percent(c.coverage||0)}</strong><span>tests exploitables</span></div></div>
      <div class="grid-intelligence section-gap"><section class="panel intelligence-main"><div class="panel-title"><div><h2>Chaînes explicatives automatiques</h2><div class="panel-sub">Le score n’affirme jamais une causalité certaine : il mesure la compatibilité géographique, temporelle et comportementale.</div></div></div>${top.length?top.map(x=>`<article class="context-card ${x.status}"><div class="context-card-head"><strong>${U().escapeHtml(x.title)}</strong><span>${U().escapeHtml(x.label)} · ${U().integer(x.score)}/100</span></div><p>${U().escapeHtml(x.summary)}</p><ol>${(x.chain||[]).map(v=>`<li>${U().escapeHtml(v)}</li>`).join('')}</ol>${(x.alternatives||[]).length?`<div class="context-alt"><strong>Causes concurrentes</strong> · ${(x.alternatives||[]).slice(0,3).map(a=>`${U().escapeHtml(a.label)} (${U().integer(a.strength)}/100)`).join(' · ')}</div>`:''}</article>`).join(''):'<div class="empty-mini">Aucune compatibilité travaux significative actuellement.</div>'}</section>
      <aside class="intel-side"><section class="panel"><div class="panel-title"><h2>Événements les plus reliés</h2><span class="panel-sub">Sources publiques Clermont</span></div><div class="alert-list">${events.length?events.map(e=>`<div class="alert-row"><span class="alert-dot"></span><div><strong>${U().escapeHtml(e.work.place||e.work.sector)}</strong><p>${U().escapeHtml(e.work.text||'')}<br>Relié à ${e.diagnostics.length} diagnostic(s) · score max ${e.maxScore}/100.</p></div></div>`).join(''):'<div class="empty-mini">Aucun événement relié.</div>'}</div></section><section class="panel section-gap"><h2>Règle anti-faux-positif</h2><p class="muted">Un chantier n’est jamais retenu seul. Le moteur exige aussi un signal commercial, une concentration géographique ou un écart aux zones témoins, et teste stock, prix, saisonnalité et météo comme explications concurrentes.</p></section></aside></div>`;
  }
