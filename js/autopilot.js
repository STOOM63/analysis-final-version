...
AU.autopilot = (() => {
  ...
  function run(model){
    ...
    if(causal){
      add('causal-context-engine','Couplage travaux ↔ diagnostics exécuté',`${causal.tested} diagnostic(s) testés · ${causal.strong} compatibilité(s) forte(s) · ${causal.moderate} moyenne(s)`,causal.top.slice(0,10).map(x=>`${x.title} · ${x.label} ${x.score}/100 · ${x.topZone||'zone non dominante'}${x.works?.[0]?` · ${x.works[0].place||x.works[0].sector}`:''}`),causal.strong?'critical':causal.moderate?'warning':'positive',{diagnostics:causal.top.map(x=>x.findingId)});
      const causalOps=causal.top.filter(x=>['strong','moderate'].includes(x.status)).slice(0,8);
      add('causal-action-plan','Plan de vérification contextuelle généré',causalOps.length?`${causalOps.length} chaîne(s) explicative(s) suivies automatiquement`:'Aucune chaîne travaux suffisamment solide à suivre',causalOps.map(x=>`${x.topZone||'Zone'} · ${x.title} · alternatives : ${(x.alternatives||[]).slice(0,2).map(a=>a.label).join(', ')||'aucune cause concurrente forte'}`),causalOps.some(x=>x.status==='strong')?'warning':'info');
    }
