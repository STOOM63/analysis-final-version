...
AU.intelligence = (() => {
  ...
  function answerQuestion(model, rawQuery) {
    ...
    if (has('travaux', 'chantier', 'circulation', 'acces', 'accès', 'mobilite', 'mobilité', 'cause externe', 'contexte clermont')) {
      if (AU.causalContext?.answer) return AU.causalContext.answer(model);
    }
    if (has('zone', 'secteur', 'geograph', 'géograph', 'brezet', 'brézet', 'clermont est')) {
