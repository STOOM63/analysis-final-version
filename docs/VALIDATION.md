# Validation de référence — Analysis Ultimate 3.2.0 ULTIMATE CAUSAL CONTEXT
...
## Validation Causal Context Engine

Le test Node `tests/test_causal_context.js` construit un scénario synthétique contrôlé dans lequel une zone Est décroche fortement alors qu’une zone Ouest témoin reste stable et qu’un chantier « route barrée » démarre à la date de rupture. Le moteur doit :

- exécuter automatiquement le test contextuel ;
- identifier la zone Est comme zone contributrice ;
- parser correctement la date de début du chantier ;
- produire au minimum une compatibilité moyenne ;
- conserver la formulation comme signal contextuel et non preuve certaine.

La v3.2 ajoute aussi :

- affinité géographique chantier ↔ micro-zone ;
- historique des événements travaux actifs et terminés ;
- analyse avant/après démarrage ou fin lorsque l’échantillon le permet ;
- causes concurrentes stock / prix / calendrier / météo / remises ;
- bloc contextuel sous chaque conclusion autonome ;
- écran Contexte causal ;
- actions Autopilot issues du couplage.
